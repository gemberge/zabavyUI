App.Views.MatchEdit = Backbone.View.extend({
	...
	render: function(data) {
		...
		this.scoresToRemove = [];
		var scoresView = new App.Views.ScoresEmbeddedInMatch({collection: this.scores, scoresToRemove: this.scoresToRemove});
	...
	addNewScore: function (user) {
		...
		if(!exist) {
			var newScore = new App.Models.Score({match: this.model.toJSON(), user: user});
	...
	cloneEntity: function() {
		...
		var view = this;
		var newScore;
		var newScores = new App.Collections.Scores();
		view.scores.each(function (score) {
			newScore = new App.Models.Score({match: clone.toJSON(), user: score.get("user")});
			newScores.add(newScore);
		});
		var matchEditView = new App.Views.MatchEdit({model: clone});
